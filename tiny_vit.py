...
try:
    from SparseAttention.sparse_attention import \
        SparseAttention as SparseSelfAttention
except (ImportError, ModuleNotFoundError):
    SparseSelfAttention = None
...
class Attention(torch.nn.Module):
    def __init__(self, dim, key_dim, num_heads=8,
                 ...
                 sparse_attn_mode: str = 'all',
                 sparse_attn_ctx: Optional[int] = None,
                 ):
        super().__init__()
        # (h, w)
        ...
        if self.use_sparse_attn:
            if SparseSelfAttention is None:
                raise ImportError(
                    'SparseAttention package is not available. '
                    'Install it or disable sparse attention.'
                )
            self.sparse_attn = SparseSelfAttention(
                num_heads=num_heads,
                attn_mode=sparse_attn_mode,
                local_attn_ctx=sparse_attn_ctx,
                causal=False,
            )
        else:
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        if self.sparse_attn is not None:
            context = self.sparse_attn(q, k, v, attn_bias=bias)
        else:
            attn = (q @ k.transpose(-2, -1)) * self.scale + bias
            attn = attn.softmax(dim=-1)
            context = attn @ v

        x = context.transpose(1, 2).reshape(B, N, self.dh)
        x = self.proj(x)
        return x
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
                 ...
                 local_conv_size=3,
                 layer_lr_decay=1.0,
                 use_sparse_window_attn=False,
                 sparse_window_attn_mode='all',
                 sparse_window_attn_ctx=None,
                 ):
        super().__init__()
        ...
        def _expand_to_layers(value, name):
            if isinstance(value, (list, tuple)):
                if len(value) != self.num_layers:
                    raise ValueError(
                        f"{name} must have length {self.num_layers}, "
                        f"but got {len(value)}."
                    )
                return list(value)
            if value is None:
        ...
        sparse_flags = _expand_to_layers(
            use_sparse_window_attn, 'use_sparse_window_attn')
        sparse_modes = _expand_to_layers(
            sparse_window_attn_mode, 'sparse_window_attn_mode')
        sparse_ctxs = _expand_to_layers(
            sparse_window_attn_ctx, 'sparse_window_attn_ctx')
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer = BasicLayer(
                    ...
                    drop=drop_rate,
                    local_conv_size=local_conv_size,
                    use_sparse_attn=sparse_flags[i_layer],
                    sparse_attn_mode=sparse_modes[i_layer],
                    sparse_attn_ctx=sparse_ctxs[i_layer],
                    **kwargs)
...
def tiny_vit_21m_224(pretrained=False, **kwargs):
    model_kwargs = dict(
        ...
        window_sizes=[7, 7, 14, 7],
        drop_path_rate=0.2,
        use_sparse_window_attn=[False, True, True, True],
        sparse_window_attn_mode=['all', 'local', 'local', 'local'],
        sparse_window_attn_ctx=[None, 12, 24, 12],
    )
    model_kwargs.update(kwargs)
